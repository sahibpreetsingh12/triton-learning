...
## 📌 Code Comment / Docstring for `row_start` and `col_offsets`

**Example:**
Matrix shape = **M=3 rows × N=4 columns**
...
* In flat memory, each new row starts `N` elements after the previous one.
  ...
  * Row 1 start = `1 * 4 = 4`
  * Row 2 start = `2 * 4 = 8`
* So for `row_id=1`, `row_start = 4` → points to `a10` in memory.

---

### **2️⃣ Why `col_offsets = tl.arange(0, BLOCK_SIZE)`**

* Generates a **vector** of column indices inside the tile.
  Example: `BLOCK_SIZE = 4` → `col_offsets = [0, 1, 2, 3]`
* Allows Triton to load/store multiple elements **in parallel** (one per lane).

---

### **3️⃣ Why `X_ptr + row_start + col_offsets`**

* `X_ptr` → pointer to the start of the entire matrix.
* `row_start` → jump to the start of the current row.
* `col_offsets` → move to the correct columns within this row’s tile.
* Adding them gives the **vector of addresses** for the tile:

  * For `row_id=1`, `row_start=4`, `col_offsets=[0,1,2,3]` → `[4,5,6,7]`
    → corresponds to `[a10, a11, a12, a13]` in flat memory.
...
* `row_id * N` → stride to the correct row start.
* `col_offsets` → stride within the row to pick the tile’s columns.
